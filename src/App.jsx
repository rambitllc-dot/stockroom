...
function Scanner({ onDetect, onClose }) {
  ...
  const [error, setError] = useState(''),
    [hint, setHint] = useState('Point camera at a QR or barcode');
  ...
  useEffect(() => {
    if (!window.jsQR) {
      ...
      s.onload = () => setHint('Ready — aim at code');
          ...
          <div style={{ display: 'grid', gap: 14 }}>
            ...
            {customFields.length > 0 && (
              <div style={{ borderTop: '1px solid #2a2a2a', paddingTop: 14 }}>
                ...
                <div style={{ display: 'grid', gap: 14 }}>
                  {customFields.map((f) => (
                    <Field key={f.id} f={f} value={form[f.key]} onChange={set} />
                  ))}
                </div>
              </div>
            )}
...
function ItemDetail({ item, customFields, onClose, onEdit, onDelete, deleting }) {
  ...
  const margin =
    item.price && item.cost ? (((item.price - item.cost) / item.price) * 100).toFixed(1) : null;
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 60, overflowY: 'auto', padding: 16 }}>
        ...
        >
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {item.category?.toUpperCase()} · {item.sku || 'NO SKU'}
                </div>
                <div
                  style={{
                    fontFamily: "'Bebas Neue',sans-serif",
            ...
            <div style={{ marginTop: 14, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <span
                style={{
                  ...
                  fontFamily: 'monospace',
                  fontWeight: 700,
                  background: isOut ? '#3b0000' : isLow ? '#451a03' : '#052e16',
                  color: isOut ? '#ef4444' : isLow ? '#f59e0b' : '#4ade80',
                  border: `1px solid ${isOut ? '#7f1d1d' : isLow ? '#92400e' : '#14532d'}`,
                }}
              >
                {isOut ? '⛔ OUT OF STOCK' : isLow ? '⚠ LOW STOCK' : '✓ IN STOCK'} — {item.quantity}{' '}
                units
              </span>
              {margin && (
                <span
          ...
          <div style={{ padding: '0 24px' }}>
            <Row
              label='Sale Price'
              val={item.price ? `$${item.price.toFixed(2)}` : null}
              accent='#4ade80'
            />
            ...
            <Row
              label='Expiry Date'
              val={item.expiry}
              accent={item.expiry && new Date(item.expiry) < new Date() ? '#ef4444' : undefined}
            />
            ...
            {item.notes && (
              <div style={{ padding: '12px 0', borderBottom: '1px solid #1e1e1e' }}>
                ...
                <div style={{ color: '#d1d5db', fontSize: 13, lineHeight: 1.5 }}>{item.notes}</div>
              </div>
            )}
            {customFields
              .filter((f) => item[f.key] != null && item[f.key] !== '')
              .map((f) => (
                <Row key={f.id} label={f.label} val={String(item[f.key])} />
              ))}
          </div>
...
function FieldManager({ customFields, onSave, onClose, saving }) {
  ...
  const [newLabel, setNewLabel] = useState(''),
    [newType, setNewType] = useState('text'),
  ...
  const addField = () => {
    ...
    if (fields.find((f) => f.key === key)) {
      setErr('Field already exists');
      return;
    }
    setFields((prev) => [
      ...prev,
      { id: uid(), key, label, type: newType, required: newReq, core: false },
    ]);
    setNewLabel('');
    setNewType('text');
    setNewReq(false);
    ...
    <>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 70, overflowY: 'auto', padding: 16 }}>
        ...
        >
          ...
          <div style={{ fontSize: 10, color: '#f59e0b', letterSpacing: 2, marginBottom: 8 }}>
            CUSTOM FIELDS
          </div>
          {fields.length === 0 && (
            <p
              style={{ color: '#374151', fontSize: 12, marginBottom: 12, fontFamily: 'monospace' }}
            >
          ...
          {fields.map((f) => (
            ...
            >
              <div>
                ...
                <span
                  style={{ color: '#6b7280', fontSize: 11, fontFamily: 'monospace', marginLeft: 8 }}
                >
                  ({f.type}){f.required ? ' *' : ''}
                </span>
              </div>
          ...
          >
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              <input
                placeholder='Field name (e.g. "Warranty Miles")'
                value={newLabel}
                onChange={(e) => {
              ...
              {err && (
                <div style={{ color: '#ef4444', fontSize: 12, fontFamily: 'monospace' }}>{err}</div>
              )}
              <button
                onClick={addField}
                style={{
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: 12 }}>
        ...
        {item.price ? (
          <div style={{ fontSize: 11, color: '#6b7280' }}>${item.price.toFixed(2)}</div>
        ) : null}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const handleScan = (code) => {
    ...
    if (found) {
      ...
      setEditItem(null);
      setShowForm(true);
      showToast('New item from scan', 'info');
    }
  };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `stockroom-${today()}.json`;
    a.click();
      ...
      >
        ...
        {view === 'dashboard' && (
          <div style={{ padding: 16, animation: 'fadeIn .3s ease' }}>
            ...
            >
              {[
                { label: 'Total Items', val: items.length, color: '#38bdf8' },
                {
                  label: 'Inventory Value',
                  val: `$${totalValue.toLocaleString('en-US', { minimumFractionDigits: 2 })}`,
                  color: '#4ade80',
                },
                {
                  label: 'Low Stock',
            ...
            {(outOfStock.length > 0 || lowStockItems.length > 0) && (
              <div style={{ marginBottom: 20 }}>
                ...
                {[
                  ...outOfStock.map((i) => ({ ...i, _t: 'out' })),
                  ...lowStockItems.map((i) => ({ ...i, _t: 'low' })),
                ].map((item) => (
                  <div
                    key={item.id}
                    onClick={() => setDetailItem(item)}
                    style={{
                      background: item._t === 'out' ? '#130000' : '#130a00',
                      border: `1px solid ${item._t === 'out' ? '#7f1d1d' : '#78350f'}`,
                      borderRadius: 12,
                      padding: '12px 16px',
                      marginBottom: 8,
                      cursor: 'pointer',
                  ...
                  >
                    <div>
                      <div style={{ color: '#f3f4f6', fontSize: 14 }}>{item.name}</div>
                    ...
                    <span
                      style={{
                        fontFamily: "'Bebas Neue',sans-serif",
                        fontSize: 16,
                        color: item._t === 'out' ? '#ef4444' : '#f59e0b',
                      }}
                    >
                      {item._t === 'out' ? 'OUT' : `${item.quantity} left`}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div style={{ padding: 16, animation: 'fadeIn .3s ease' }}>
              ...
              {['All', ...CATEGORIES].map((c) => (
                <button
                  ...
                  style={{
                    whiteSpace: 'nowrap',
                    padding: '5px 14px',
                    background: filterCat === c ? '#f59e0b' : '#161616',
                    border: '1px solid ' + (filterCat === c ? '#f59e0b' : '#2d2d2d'),
                    borderRadius: 20,
                    color: filterCat === c ? '#000' : '#9ca3af',
                    cursor: 'pointer',
                    fontSize: 11,
                    fontFamily: 'monospace',
                  }}
            ...
            {filtered.map((item, i) => (
              <div key={item.id} className='item-card' style={{ animationDelay: `${i * 0.04}s` }}>
                <ItemRow item={item} onClick={() => setDetailItem(item)} />
              </div>
            ))}
          </div>
        )}
        ...
        {view === 'data' && (
          <div style={{ padding: 16, animation: 'fadeIn .3s ease' }}>
            ...
            >
              <div style={{ fontSize: 10, color: '#f59e0b', letterSpacing: 2, marginBottom: 6 }}>
                CUSTOM FIELDS
              </div>
      ...
      {toast && (
        <div
          style={{
            ...
            transform: 'translateX(-50%)',
            background:
              toast.type === 'error' ? '#1a0000' : toast.type === 'info' ? '#0a1628' : '#052e16',
            border: `1px solid ${toast.type === 'error' ? '#7f1d1d' : toast.type === 'info' ? '#1e3a5f' : '#14532d'}`,
            color:
              toast.type === 'error' ? '#fca5a5' : toast.type === 'info' ? '#7dd3fc' : '#86efac',
            borderRadius: 12,
            padding: '10px 20px',
            fontSize: 13,
            fontFamily: 'monospace',
